...
export const Init = () : void =>
{
    ...
    loggedInMenuToggle.addEventListener("click", () => {
        if (loggedInNavMenu.style.display === "flex") HideHeaderMenu();
        else Utilities.ShowElement(loggedInNavMenu, "flex");
    });
...
const HandlePreferredColorSchemeChange = () =>
{
    document.querySelectorAll("meta[name=theme-color], meta[name=msapplication-navbutton-color]")
        .forEach(element => (<HTMLMetaElement>element).content = getComputedStyle(document.documentElement).getPropertyValue("--primary-color").trim());
